...
def letter_messages(item: dict[str, Any], candidate: dict[str, Any]) -> list[dict[str, str]]:
    ...
    system = (
        ...
        "2) Одна фраза живого интереса к роли. Если направление вакансии — смежное с основным "
        "опытом кандидата (например вакансия про DevOps/инфраструктуру, а ядро опыта — AI/backend), "
        ...
        "3) Абзац с КОНКРЕТНЫМ релевантным опытом под эту вакансию. ОБЯЗАТЕЛЬНО приведи не менее "
        "двух конкретных фактов из профиля — с цифрами-результатами или с названиями "
        "технологий/проектов, которые реально относятся к этой вакансии (например для AI: снизил "
        "ручную модерацию с 400–1000 до ~30 в день; поднял точность выбора инструмента с 57% до "
        "90%; для DevOps/инфраструктуры: Kubernetes и GitLab CI/CD в проде, self-hosted на Proxmox, "
        "WireGuard, мониторинг Grafana/Loki). Показывай суть и эффект, а не список баззвордов. "
        "Названия компаний, продуктов и проектов пиши ТОЧНО как в профиле. Только то, что есть в "
        "профиле — ничего не выдумывай.\n"
        "4) Только если роль продуктовая / про внедрение (AI, автоматизация) — можно добавить "
        "короткую конкретную идею: что бы ты сделал у них и какой измеримый результат ожидать. "
        "Для чисто инфраструктурных/эксплуатационных ролей этот абзац не нужен.\n"
